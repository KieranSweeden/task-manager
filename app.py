...
def profile(username):
    # grab the session iser's username from the database
    username = mongo.db.users.find_one(
        {"username": session["user"]})["username"]
    
    # Is a user cookie is present, render profile page
    if session["user"]:
...
def add_task():
    # if request is post, send request data to database
    if request.method == "POST":
        ...
        # to_dict() creates a dictionary using the values within form
...
def edit_task(task_id):
    if request.method == "POST":
        ...
        # 1st being the doc to be edited sent as dictionary
        # 2nd being the data to be submitted to the doc, as a dictionary
